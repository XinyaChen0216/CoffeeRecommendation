...
public class CoffeeDao {
	...
	public List<Coffee> getCoffeeByOrigin(CoffeeOrigins coffeeOrigin) throws SQLException {
		...
		try {
			...
			selectStmt.setString(1, coffeeOrigin.getOrigin().getOriginName());
			results = selectStmt.executeQuery();
			if(results.next()) {
	...
	public List<Coffee> getCoffeeByRoaster(String roasterName) throws SQLException {
    List<Coffee> coffees = new ArrayList<Coffee>();
    String selectCoffees =
      "SELECT CoffeeName,RoastType,Price,RoasterName " +
      "FROM Coffee " +
	...
	public List<Coffee> getCoffeesWithInPrice(Double maxPrice) throws SQLException {
		...
		try {
			...
			while(results.next()) {
				String coffeeName = results.getString("CoffeeName");
			    String roastTypeStr = results.getString("RoastType");
			    Coffee.RoastType roastType;
			    try {
			        roastType = Coffee.RoastType.valueOf(roastTypeStr); // Converting to enum
			    } catch (IllegalArgumentException | NullPointerException e) {
			        // Handle the case where roastTypeStr is not a valid RoastType
			        roastType = Coffee.RoastType.Medium; // Assuming Medium
			        // Or handle the exception as required
			    }

			    Double price = results.getDouble("Price");
			    String roasterName = results.getString("RoasterName");
			    Coffee selectCoffee = new Coffee(coffeeName, roastType, price, roasterName);
